...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request
{
    ...
    if ([[self supportedSchemes] containsObject:[[request URL] scheme]])
    {
        ...
        if (([urlString rangeOfString:@"&_t_=multi"].location != NSNotFound) || ([urlString rangeOfString:@"&_t_=single"].location != NSNotFound)) { // #multi#   #single#
            NSLog(@"--request进入处理流程--");
            return YES;
        }
        else {
...
- (void)multiCacheHandlerWith:(NSString *)fileName {
    ...
    if ([[self class] isCacheExistWith:md5_fileName]) {
        ...
        NSURLRequest * req = [NSURLRequest requestWithURL:[NSURL URLWithString:fileName]];
...
- (void)singleCacheHandlerWith:(NSString *)fileName {
    ...
    if ([[self class] isCacheExistWith:md5_fileName]) {
        ...
        NSURLRequest * req = [NSURLRequest requestWithURL:[NSURL URLWithString:tempName]];
        NSURLConnection *connection = [NSURLConnection connectionWithRequest:req
                                                                    delegate:self];
