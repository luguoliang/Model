...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if ([HTMLSource rangeOfString:@"404"].location != NSNotFound || [HTMLSource rangeOfString:@"502"].location != NSNotFound || [HTMLSource rangeOfString:@"403"].location != NSNotFound ) {
        [self performSelector:@selector(webviewErrorHandler) withObject:nil afterDelay:1.0];
    }
}
- (void)webviewErrorHandler {
    [self UI_LoadFinish:@"error" :@{}];
}
...
- (void)UI_LoadFinish:(NSString *)reqId :(NSDictionary *)params {
    if ([_delegate respondsToSelector:@selector(loadingFinish:reqId:)]) {
        [_delegate loadingFinish:params reqId:reqId];
    }
}
...
- (void)UI_NavTitle:(NSString *)reqId :(NSDictionary *)params {
    BaoFooTitleModel * model = [[BaoFooTitleModel alloc] init];
...
- (UIButton *)setBarBtnItemWith:(BaoFooBtnModel *)model Target:(id)target action:(SEL)action {
    ...
    if ([NSStringFromSelector(action) isEqualToString:@"rightBtnAction:"] || [NSStringFromSelector(action) isEqualToString:@"closeBtnAction:"]) {
