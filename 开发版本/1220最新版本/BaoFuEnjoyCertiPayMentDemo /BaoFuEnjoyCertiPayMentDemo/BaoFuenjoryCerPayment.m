...
@interface BaoFuenjoryCerPayment ()<BaoFooJSWebViewManagerDelegate,abc>
{
    BaoFooJSWebViewManager*_manager;
    UIView*_baofuWebView;
    BaoFuenjoryCerPayment*_newWeb;
    NSString*_urlStr;
    Agreement*_testController;
...
- (void)openNewWebview:(NSString *)url{
    ...
    _testController = [[Agreement alloc] init];
    _testController.urlStr = url;
    [[BaoFuBufferView sharedManager] showBufferAddedTo:self.view animated:YES];
}
//加载成功后回调，弹出sdk
- (void)loadingFinish:(NSDictionary *)params reqId:(NSString *)reqId{
    NSLog(@"_newWeb002 = %@",self);
    if ([reqId isEqualToString:@"error"]) {
        [[BaoFuBufferView sharedManager] hideBufferForView:self.view animated:YES];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:urlkey];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"1"];
    }
    else{
        if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"1"] isEqualToString:@"1"]) {
            [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"1"];
            [[BaoFuBufferView sharedManager] hideBufferForView:self.view animated:YES];
            [self presentViewController:_testController animated:YES completion:^{
            }];
        }
        else{
